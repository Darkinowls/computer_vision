...
def ver2():
    edges_teach = get_edges_array('source/teach_dataset')

    np.savetxt('first_foto.txt', edges_teach[0], fmt='%1.d')

    # 4d => num_foto, width, height, color_num_channel
    edges_teach = edges_teach.reshape(edges_teach.shape[0], 200, 200, 1)

    classes_int_teach, classes = get_class_array('source/teach_dataset')

    model = mind_model()

    model.fit(edges_teach, classes_int_teach, epochs=20)

    edges_test = get_edges_array('source/test_dataset')
    ...
    print(prs_teach)
    print('-' * 30)
    print('Навчання на teach_dataset завершено!')
    print('Випробовуємо модель на test_dataset:')
